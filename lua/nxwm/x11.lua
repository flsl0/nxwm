...
function M.dpyerr() error('X display not open') end
M.code_to_name={
...
function M.win_set_key(win,key,mods)
    if not M.display then M.dpyerr() end
    xlib.XGrabKey(M.display,M.key_get_key(key),M.key_get_mods(mods),win,0,xlib.GrabModeAsync,xlib.GrabModeAsync)
end
...
function M.start()
    if M.display then return end
    local display=xlib.XOpenDisplay(nil)
    if display==nil then error('X display open error') end
...
function M.stop()
    if not M.display then return end
...
function M.step()
    if not M.display then M.dpyerr() end
    if xlib.XPending(M.display)==0 then return end
